...
if "Overview" in page:
    ...
    st.markdown('<div class="card cg">Classifies by majority vote among <b>k nearest neighbors</b> '
                ...
                'Best k=37 (regression) → <b>R²=0.786</b>.</div>',unsafe_allow_html=True)
    st.markdown('<div class="card cb">',unsafe_allow_html=True)
    st.subheader("📒 Notebook Results (Verified)")
    mbox({"Best k (Cls)":"22","Accuracy (80:20)":"63.09%","5-Fold CV":"61.9%",
          "Best k (Reg)":"37","R² (80:20)":"0.786"})
    c1,c2=st.columns(2)
    with c1:
        ...
        st.dataframe(pd.DataFrame([
            ...
            {"Split":"5-Fold CV","K":34,"Accuracy":"61.9%","F1":"—"},
        ]),use_container_width=True,hide_index=True)
    with c2:
        st.markdown("**Regression**")
        st.dataframe(pd.DataFrame([
            {"Split":"80:20","K":37,"R²":"0.786","MAE":"13.79 cm"},
            {"Split":"70:30","K":36,"R²":"0.788","MAE":"13.84 cm"},
            {"Split":"50:50","K":28,"R²":"0.785","MAE":"13.95 cm"},
            {"Split":"5-Fold CV","K":35,"R²":"0.782","MAE":"—"},
        ]),use_container_width=True,hide_index=True)
    st.markdown("</div>",unsafe_allow_html=True)
    ...
    st.markdown('<div class="card cg">Cost-complexity pruning (α) prevents overfitting. '
                '<b>Classification:</b> α=0.000377 → depth 14, 477 nodes → <b>72.15%</b>. '
    ...
    c1,c2=st.columns(2)
    with c1:
        st.markdown("**Classification** (α=0.000377, depth=14, nodes=477)")
        st.dataframe(pd.DataFrame([
            {"Split":"80:20","Test Acc":"72.15%","Train Acc":"78.04%"},
            {"Split":"70:30","Test Acc":"70.58%","Train Acc":"79.08%"},
            {"Split":"50:50","Test Acc":"67.06%","Train Acc":"85.96%"},
            {"Split":"5-Fold CV","Test Acc":"70.39% ±1.39%","Train Acc":"—"},
        ]),use_container_width=True,hide_index=True)
        mbox({"Best Acc":"72.15%","CV Acc":"70.39%"})
    with c2:
        st.markdown("**Regression** (α=16.21, depth=3, nodes=13)")
        st.dataframe(pd.DataFrame([
            {"Split":"80:20","R²":"0.6768","RMSE":"22.86 cm"},
            {"Split":"70:30","R²":"0.6699","RMSE":"23.09 cm"},
            {"Split":"50:50","R²":"0.6852","RMSE":"22.47 cm"},
            {"Split":"5-Fold CV","R²":"0.6947 ±0.015","RMSE":"—"},
        ]),use_container_width=True,hide_index=True)
        mbox({"Best R²":"0.6768","CV R²":"0.695"})
    st.markdown("</div>",unsafe_allow_html=True)
    tc,tr=st.tabs(["🎯 Run Classification","📈 Run Regression"])
    with tc:
        c1,c2=st.columns(2)
    ...
    st.markdown('<div class="card cb">',unsafe_allow_html=True)
    st.subheader("📒 Notebook Results (Verified)")
    mbox({"Linear (full)":"63.13%","RBF Tuned (full)":"71.27%","Best Params":"C=10, γ=0.1","Kernel":"RBF"})
    st.dataframe(pd.DataFrame([
        {"Dataset":"Full (13,385)","Kernel":"Linear","C":"—","Accuracy":"63.13%","Note":"Baseline"},
        {"Dataset":"Full (13,385)","Kernel":"RBF","C":"10","Accuracy":"71.27%","Note":"Grid Search (best)"},
        {"Dataset":"Sample (1,000)","Kernel":"RBF","C":"1","Accuracy":"84.67%","Note":"70:30 — small sample"},
        {"Dataset":"Sample (500)","Kernel":"RBF","C":"1","Accuracy":"79.60%","Note":"50:50 — small sample"},
    ]),use_container_width=True,hide_index=True)
    st.markdown('<div class="wbox">📌 84.67% was on a 1,000-sample subset. '
                'Full dataset with CV gives <b>~70–71%</b>.</div>',unsafe_allow_html=True)
    st.markdown("</div>",unsafe_allow_html=True)
    ...
    st.markdown('<div class="card cg">Multi-Layer Perceptron — 256→128→64 units, ReLU, '
                ...
                'Corrected sklearn MLP gives realistic <b>~73–74%</b>.</div>',unsafe_allow_html=True)
    st.markdown('<div class="card cb">',unsafe_allow_html=True)
    ...
    st.dataframe(pd.DataFrame([
        {"Model":"KNN (k=22)","Best Acc":"63.09%","F1":"0.63","Split":"80:20","CV":"61.9%","Rank":"5th"},
        {"Model":"Decision Tree (α=0.000377)","Best Acc":"72.15%","F1":"0.71","Split":"80:20","CV":"70.39%","Rank":"2nd ✅"},
        {"Model":"SVM (RBF, C=10, γ=0.1)","Best Acc":"71.27%","F1":"0.71","Split":"80:20","CV":"~70.8%","Rank":"3rd ✅"},
        {"Model":"Neural Network (MLP)","Best Acc":"~74%","F1":"~0.73","Split":"80:20","CV":"~73%","Rank":"1st ⭐"},
    ...
    st.dataframe(pd.DataFrame([
        ...
        {"Model":"Neural Network (MLP)","Best R²":"0.802","RMSE":"17.77 cm","CV R²":"0.794","Rank":"2nd ✅"},
        {"Model":"SVM (SVR, RBF)","Best R²":"0.79","RMSE":"~18.4 cm","CV R²":"~0.79","Rank":"3rd ✅"},
    ...
    with c1:
        bar_chart(["KNN","Dec.Tree","SVM","MLP","Lin.Reg"],
                  [63.09,72.15,71.27,74.0,50.0],"Best Accuracy (%)","Classification Accuracy",
                  color=PAL+[LBLUE],hline=(70,"#1D9E75","70% line"))
    with c2:
        bar_chart(["Lin.Reg","MLP","SVR","KNN","Dec.Tree"],
                  [.8033,.802,.79,.786,.677],"R² Score","Regression R²",
    ...
    scores_r={"KNN":[63,65,63,63,62],"Dec.Tree":[72,72,71,71,70],
              "SVM":[71,71,71,71,71],"MLP":[74,73,74,73,73]}
...
**Best classification: MLP ≈ 74%** &nbsp;·&nbsp; **Best regression: Linear Reg. R²=0.803**
    """); st.markdown("</div>",unsafe_allow_html=True)
