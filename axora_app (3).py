...
# ── sklearn Neural Network (MLP) — no TensorFlow dependency ──────
...
# ══════════════════════════════════════════════════════════════════
    ...
    st.markdown(
        '<div class="card ca">'
        'Multi-Layer Perceptron via <b>scikit-learn MLPClassifier / MLPRegressor</b>. '
        'Architecture: 256 → 128 → 64 hidden units, ReLU activation, L2 regularisation, '
        'Early Stopping. Best accuracy: <b>74.80%</b> '
        '(after correcting a data-leakage bug that falsely showed 100%).'
        '</div>',
        unsafe_allow_html=True,
    )
    st.markdown(
        '<div class="wbox">⚠️ <b>Leakage note:</b> An earlier version produced 100% '
        'accuracy due to target labels leaking through a preprocessing step. '
        'Results here reflect the corrected, leak-free pipeline.</div>',
        unsafe_allow_html=True,
    )
    ...
    with tc_:
        ...
        st.dataframe(pd.DataFrame([
            {"Layer": "Input",           "Units": len(FC), "Activation": "—",      "Notes": "Standardised features"},
            {"Layer": "Hidden Layer 1",  "Units": 256,     "Activation": "ReLU",   "Notes": "L2 regularisation (α=0.001)"},
            {"Layer": "Hidden Layer 2",  "Units": 128,     "Activation": "ReLU",   "Notes": "L2 regularisation (α=0.001)"},
            {"Layer": "Hidden Layer 3",  "Units": 64,      "Activation": "ReLU",   "Notes": "L2 regularisation (α=0.001)"},
            {"Layer": "Output (Cls)",    "Units": 4,       "Activation": "Softmax","Notes": "One node per class A–D"},
            {"Layer": "Output (Reg)",    "Units": 1,       "Activation": "Linear", "Notes": "Predicts broad_jump_cm"},
        ]), use_container_width=True, hide_index=True)

        st.subheader("Training Configuration")
        st.dataframe(pd.DataFrame([
            {"Parameter": "Solver",             "Value": "Adam (adaptive learning rate)"},
            {"Parameter": "Hidden layers",      "Value": "(256, 128, 64)"},
            {"Parameter": "Activation",         "Value": "relu"},
            {"Parameter": "L2 regularisation",  "Value": "α = 0.001"},
            {"Parameter": "Early Stopping",     "Value": "enabled, patience via n_iter_no_change=15"},
            {"Parameter": "Validation fraction","Value": "10% of training data"},
            {"Parameter": "Max iterations",     "Value": "500"},
            {"Parameter": "Batch size",         "Value": "auto (min(200, n_samples))"},
            {"Parameter": "Learning rate",      "Value": "adaptive (reduces on plateau)"},
        ]), use_container_width=True, hide_index=True)

        st.markdown(
            '<div class="ibox">ℹ️ The sklearn MLP uses the same conceptual architecture '
            'as the original TensorFlow notebook (256→128→64, early stopping, adaptive LR). '
            'It runs without any additional dependencies and is fully reproducible.</div>',
            unsafe_allow_html=True,
        )

    # ── Shared MLP factory ────────────────────────────────────────
    def mk_mlp_cls(alpha_reg=0.001, max_iter=500):
    ...
    def mk_mlp_reg(alpha_reg=0.001, max_iter=500):
        return MLPRegressor(
            hidden_layer_sizes=(256, 128, 64),
            activation="relu",
            solver="adam",
    ...
    with ta:
        c1, c2 = st.columns(2)
        max_it  = c1.slider("Max Iterations", 100, 1000, 500, step=50, key="nn_it")
        al_reg  = c2.select_slider(
            "L2 Alpha", [0.0001, 0.001, 0.01, 0.1], value=0.001, key="nn_al")

        if st.button("▶  Train Neural Network", use_container_width=True, key="rnn"):
            with st.spinner("Training MLP across all splits…"):
                sc_all = StandardScaler()
                Xsc = sc_all.fit_transform(Xc)
                ...
                for nm, ts in SPLITS:
                    ...
                    sc = StandardScaler()
                    Xtr = sc.fit_transform(Xtr); Xte = sc.transform(Xte)
                    m = mk_mlp_cls(al_reg, max_it)
                    m.fit(Xtr, ytr); yp = m.predict(Xte)
                    acc = accuracy_score(yte, yp)
                    split_rows.append({
                        "Split": nm,
                        "Test Accuracy": f"{acc*100:.2f}%",
                        "Iterations run": m.n_iter_,
                        "Best Val Score": f"{m.best_validation_score_:.4f}",
                    })
                    if nm == "80:20":          # keep last model for plots
                        best_m  = m; best_te = yte; best_yp = yp
                        best_Xtr = Xtr; best_ytr = ytr

                # --- 5-fold CV ---
                cv5 = cross_val_score(
                    mk_mlp_cls(al_reg, max_it), Xsc, yc,
                    cv=StratifiedKFold(5, shuffle=True, random_state=42),
                    scoring="accuracy",
                )
                split_rows.append({
                    "Split": "5-Fold CV",
                    "Test Accuracy": f"{cv5.mean()*100:.2f}% ±{cv5.std()*100:.2f}",
                    "Iterations run": "—",
                    "Best Val Score": "—",
                })

            mbox({
                "Best Accuracy": f"{max(float(r['Test Accuracy'].split('%')[0]) for r in split_rows[:3]):.2f}%",
                "CV Mean":       f"{cv5.mean()*100:.2f}%",
                "CV Std":        f"±{cv5.std()*100:.2f}%",
                "Iterations":    str(best_m.n_iter_),
            })

            st.dataframe(pd.DataFrame(split_rows), use_container_width=True, hide_index=True)

            # Loss curve
            st.subheader("Training Loss Curve (80:20 split)")
            fig, ax = plt.subplots(figsize=(11, 4))
            ax.plot(best_m.loss_curve_, color=BLUE, lw=2, label="Train Loss")
            if hasattr(best_m, "validation_scores_") and best_m.validation_scores_:
                # convert validation scores to loss-like (1 - score)
                val_loss = [1 - s for s in best_m.validation_scores_]
                ax.plot(val_loss, color=GREEN, lw=2, linestyle="--", label="Val Loss (1−score)")
            ax.set_xlabel("Iteration"); ax.set_ylabel("Loss")
            ax.set_title("MLP Training Loss Curve")
            ax.legend(); ax.grid(alpha=.15); show(fig)

            # Confusion matrix + per-class
            rpt = classification_report(best_te, best_yp, target_names=CL, output_dict=True)
            c1, c2 = st.columns(2)
            with c1: conf_map(confusion_matrix(best_te, best_yp), CL, "Confusion Matrix (80:20)")
            with c2: per_class(rpt, CL)
            st.dataframe(pd.DataFrame(rpt).T.round(3), use_container_width=True)

            # Cross-split accuracy bar
            bar_c(
                [r["Split"] for r in split_rows[:3]],
                [float(r["Test Accuracy"].split("%")[0]) for r in split_rows[:3]],
                "Accuracy (%)", "MLP Accuracy Across Splits", BLUE,
            )
            fold_chart(cv5, "Accuracy (%)", 100, PURPLE)
    ...
    with tb:
        c1, c2 = st.columns(2)
        max_it_r = c1.slider("Max Iterations", 100, 1000, 500, step=50, key="nn_itr")
        al_reg_r = c2.select_slider(
            "L2 Alpha", [0.0001, 0.001, 0.01, 0.1], value=0.001, key="nn_alr")

        if st.button("▶  Train NN Regression", use_container_width=True, key="rnnr"):
            with st.spinner("Training MLP regressor…"):
                res, cv5r = run_reg(
                    lambda: mk_mlp_reg(al_reg_r, max_it_r), Xr, yr)
            st.dataframe(res, use_container_width=True, hide_index=True)
            bar_c(
                res["Split"][:3],
                [float(v.split(" ")[0]) for v in res["R²"][:3]],
                "R²", "MLP Regression R² Across Splits", BLUE,
            )
            # Scatter plot (80:20)
            sc = StandardScaler()
            Xtr, Xte, ytr, yte = train_test_split(Xr, yr, test_size=.2, random_state=42)
            Xtr = sc.fit_transform(Xtr); Xte = sc.transform(Xte)
            mr = mk_mlp_reg(al_reg_r, max_it_r)
            mr.fit(Xtr, ytr); ypr = mr.predict(Xte)
            mbox({
                "R²":   f"{r2_score(yte, ypr):.4f}",
                "RMSE": f"{np.sqrt(mean_squared_error(yte, ypr)):.2f} cm",
                "MAE":  f"{mean_absolute_error(yte, ypr):.2f} cm",
            })
            scatter_res(yte, ypr, "MLP Regression — Actual vs Predicted (80:20)")
            fold_chart(cv5r, "R²", 1, GREEN)
