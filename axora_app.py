...
║   Streamlit App  |  5 ML Models  |  Classification + Regression  ║
║   Team: Alaa · Amira · Aya A. · Aya E. · Aya S. · Aya Sh.       ║
╚══════════════════════════════════════════════════════════════════╝
"""
...
h2{color:#3B8BD4!important;font-size:1.45rem!important}
...
.ca{border-left:4px solid #1D9E75}.cp{border-left:4px solid #534AB7}
...
.mv{font-size:1.9rem;font-weight:800;color:#3B8BD4;line-height:1.1}
...
.ibox{background:#0A1E35;border:1px solid #1D4570;border-radius:10px;
      padding:14px 18px;margin:8px 0;color:#85B7EB;font-size:.88rem}
.wbox{background:#1E1500;border:1px solid #A07800;border-radius:10px;
      padding:14px 18px;margin:8px 0;color:#D4A800;font-size:.88rem}
.sbox{background:#0A1E14;border:1px solid #1D9E75;border-radius:10px;
      padding:14px 18px;margin:8px 0;color:#1D9E75;font-size:.88rem}
...
# ─── Logo SVG ───────────────────────────────────────────────────
LOGO = """<svg viewBox="0 0 680 420" xmlns="http://www.w3.org/2000/svg" width="220" height="136">
  <defs>
  ...
  <polygon points="340,70 374,90 374,130 340,150 306,130 306,90"
           fill="none" stroke="url(#bG2)" stroke-width="2"/>
  <polygon points="340,80 366,96 366,124 340,140 314,124 314,96" fill="#1A2A45"/>
  <line x1="340" y1="92" x2="326" y2="130" stroke="url(#bG2)"
        stroke-width="2.5" stroke-linecap="round"/>
  <line x1="340" y1="92" x2="354" y2="130" stroke="url(#bG2)"
        stroke-width="2.5" stroke-linecap="round"/>
  ...
  <text x="340" y="214" text-anchor="middle"
        ...
        font-weight="700" letter-spacing="12" fill="url(#bG2)">AXORA</text>
  <rect x="180" y="222" width="320" height="2.5" rx="1.5"
        fill="url(#aG2)" opacity=".9"/>
  ...
  <text x="340" y="336" text-anchor="middle" font-family="Georgia,serif"
        font-size="10.5" letter-spacing="1" fill="#4d7fa8" opacity=".9">
...
# ─── Colour palette ─────────────────────────────────────────────
DARK=  '#0D1B2A'; PANEL= '#0F1E33'
BLUE=  '#3B8BD4'; PURPLE='#534AB7'
...
def _style(fig):
    ...
    for ax in fig.get_axes():
        ...
        for sp in ax.spines.values(): sp.set_edgecolor('#1D4570')
    return fig

def show(fig):
    st.pyplot(_style(fig), use_container_width=True); plt.close(fig)

def mbox(d: dict):
    cols = st.columns(len(d))
    for col,(lbl,val) in zip(cols, d.items()):
        col.markdown(f'<div class="mb"><div class="mv">{val}</div>'
...
def bar_c(labels, vals, ylabel, title, color=BLUE):
    ...
    for b in bars:
        ax.text(b.get_x()+b.get_width()/2, b.get_height()+max(vals)*.012,
                f'{b.get_height():.3f}' if max(vals)<10 else f'{b.get_height():.1f}',
                ha='center', va='bottom', color='#C8D8EA', fontsize=9, fontweight='bold')
    ax.set_ylabel(ylabel); ax.set_title(title)
    ax.set_ylim(0, max(vals)*1.25+.03); ax.grid(axis='y',alpha=.2,color=LBLUE)
    show(fig)

def scatter_res(yte, yp, title="Actual vs Predicted"):
    fig,ax2 = plt.subplots(1,2,figsize=(13,4.5))
    ax2[0].scatter(yte,yp,alpha=.35,color=BLUE,s=12)
    mn,mx = min(yte.min(),yp.min()), max(yte.max(),yp.max())
    ax2[0].plot([mn,mx],[mn,mx],'--',color=RED,lw=2,label='Perfect')
    ax2[0].set_xlabel('Actual'); ax2[0].set_ylabel('Predicted')
    ax2[0].set_title(title); ax2[0].legend()
    res = yte-yp
    ax2[1].hist(res,bins=35,color=GREEN,edgecolor=DARK,alpha=.85)
    ax2[1].axvline(0,color=RED,lw=2,linestyle='--')
    ax2[1].set_xlabel('Residual'); ax2[1].set_title('Residual Distribution')
    show(fig)

def fold_chart(scores, label="Acc (%)", scale=100, color=PURPLE):
    pct = scores*scale
    fig,ax = plt.subplots(figsize=(9,3.8))
    bars = ax.bar([f'Fold {i+1}' for i in range(len(pct))],
...
def per_class(rpt, classes):
    ...
    for i,(m,c) in enumerate(zip(metrics,[BLUE,GREEN,PURPLE])):
        ...
        ax.bar(x+i*w, vals, w, label=m.capitalize(), color=c, edgecolor=DARK, zorder=3)
    ax.set_xticks(x+w); ax.set_xticklabels([f'Class {c}' for c in classes])
    ax.set_ylabel('Score'); ax.set_title('Per-Class Metrics')
    ax.set_ylim(0,1.1); ax.legend(); ax.grid(axis='y',alpha=.2)
    show(fig)
...
def run_cls(fn, X, y, scale=True):
    ...
    cv = cross_val_score(fn(),Xsc,y,
                         cv=StratifiedKFold(5,shuffle=True,random_state=42),
                         scoring='accuracy')
    rows.append({'Split':'5-Fold CV',
...
def run_reg(fn, X, y, scale=True):
    ...
    rows.append({'Split':'5-Fold CV',
        'R²':f"{cv.mean():.4f} ±{cv.std():.4f}",'RMSE':'—','MAE':'—'})
    return pd.DataFrame(rows), cv

# ─── Data ────────────────────────────────────────────────────────
@st.cache_data
def load(up=None):
    if up is not None: return pd.read_csv(up)
    ...
    return pd.DataFrame({'age':age,'gender':gnd,'height_cm':ht.clip(130,200),
        ...
        'gripForce':gr,'sit and bend forward_cm':np.random.normal(15,8,n).clip(-20,50),
...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown("**📂 Dataset**")
    up = st.file_uploader("Upload bodyPerformance.csv", type=["csv"],
                           label_visibility="collapsed")
    st.markdown("---")
    page = st.radio("**🧭 Navigation**", [
        "🏠 Overview","📊 EDA","🤖 KNN","📈 Linear Regression",
        "🌳 Decision Tree","⚙️ SVM","🧠 Neural Network",
        "🏆 Model Comparison","🔮 Live Predictor"
    ], label_visibility="collapsed")
    st.markdown("---")
    for n,c in [("Alaa Issawi","gg"),("Amira Salama","bg"),
                ("Aya Abdel Maksoud","pg"),("Aya El-Sabi","gg"),
                ("Aya Imam","bg"),("Aya Khalil","pg")]:
        st.markdown(f'<span class="{c}">{n}</span>', unsafe_allow_html=True)
    st.markdown('<br><span class="rg">Intro to AI & ML · 2024–2025</span>',
                unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════
if "Overview" in page:
    st.markdown(f"""<div class="cover">{LOGO}
    <div>
      <div style="font-size:2rem;font-weight:900;color:#3B8BD4">
        Body Performance Analytics</div>
      ...
      <div style="margin-top:12px">
        <span class="gg">KNN</span><span class="bg">Linear Regression</span>
        <span class="pg">Decision Tree</span><span class="rg">SVM</span>
        <span class="gg">Neural Network</span></div>
    </div></div>""", unsafe_allow_html=True)

    mbox({"Records":f"{len(df):,}","Features":len(FC),
          "Classes":4,"ML Models":5})
    ...
    with c3:
        st.dataframe(df[['age','weight_kg','gripForce',
                          'sit-ups counts','broad jump_cm']].describe().round(1),
...
# ══════════════════════════════════════════════════════════════════
    ...
    t1,t2,t3,t4,t5=st.tabs(["📈 Distributions","📦 Boxplots",
                              "🔗 Correlations","🎯 By Class","🔍 Insights"])
    with t1:
        st.subheader("Numeric Distributions")
        fig,ax=plt.subplots(2,5,figsize=(20,8)); ax=ax.flatten()
        for i,c in enumerate(NUM):
            ax[i].hist(raw[c].dropna(),bins=35,color=PAL[i%4],alpha=.85,edgecolor=DARK)
            ax[i].set_title(c,fontsize=9)
        plt.tight_layout(pad=1.5); show(fig)
        st.markdown('<div class="ibox">📌 age: right-skewed; height_cm: bimodal (M/F);'
                    ' broad_jump: left-skewed; BP columns: near-normal.</div>',
                    unsafe_allow_html=True)

    with t2:
        st.subheader("Boxplots — Standardised")
        scv=StandardScaler()
        sc_df=pd.DataFrame(scv.fit_transform(raw[NUM].dropna()),columns=NUM)
        fig,ax=plt.subplots(figsize=(16,5))
        sc_df.boxplot(ax=ax,patch_artist=True,notch=False,
        ...
        ax.set_title("Standardised Boxplots"); plt.xticks(rotation=35,ha='right',fontsize=8)
        show(fig)
        oc=[]
        for c in NUM:
            q1,q3=raw[c].quantile(.25),raw[c].quantile(.75); iqr=q3-q1
            n=((raw[c]<q1-1.5*iqr)|(raw[c]>q3+1.5*iqr)).sum()
            oc.append({'Feature':c,'Outliers':n,'%':f"{n/len(raw)*100:.2f}%"})
    ...
    with t3:
        ...
        ax2.barh(cr.index,cr.values,
                 color=[RED if v<0 else GREEN for v in cr.values],edgecolor=DARK)
        ax2.axvline(0,color=LBLUE,lw=1)
        ax2.set_title("Pearson r with Performance Class"); ax2.set_xlabel("r")
        show(fig2)

    with t4:
        sel=st.selectbox("Feature",NUM,key="eda_sel")
        fig,ax=plt.subplots(1,2,figsize=(14,5))
        for cls_,col_ in zip(CL,PAL):
            sub=raw[raw['class']==cls_][sel]
            ax[0].hist(sub,bins=25,alpha=.55,label=f'Class {cls_}',color=col_)
        ax[0].set_title(f"{sel} — by Class"); ax[0].legend(); ax[0].set_xlabel(sel)
        bp=ax[1].boxplot([raw[raw['class']==c][sel].dropna() for c in CL],
                          labels=CL,patch_artist=True,notch=False)
        for p,c in zip(bp['boxes'],PAL): p.set_facecolor(c); p.set_alpha(.7)
        for md in bp['medians']: md.set_color('white'); md.set_linewidth(2)
    ...
    with t5:
        ...
        for title_,body_,cls_ in [
            ("✅ Balanced Dataset",
             "~25% per class — no SMOTE needed.","ca"),
            ("📐 Strongest Predictors",
             "sit_and_bend (r=−0.61), sit_ups (r=−0.45), broad_jump (r=−0.26).","ca"),
            ("⚠️ Low-Signal Features",
             "diastolic & systolic BP: r<0.07 with class.","cp"),
            ("👥 Gender Effect",
             "height_cm & gripForce are bimodal; males jump ~40–50 cm further.","card"),
            ("📅 Age Skew",
             "Age right-skewed (0.60); older → lower fitness class.","cr"),
        ]:
            st.markdown(f'<div class="card {cls_}"><b style="color:#3B8BD4">'
                        f'{title_}</b><br><span style="font-size:.9rem">'
                        f'{body_}</span></div>', unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════
    ...
    st.markdown('<div class="card ca">KNN classifies by majority vote among '
                '<b>k nearest neighbors</b>. StandardScaler applied. '
                'k=1–50 evaluated. Best k=<b>22</b> on 80:20 → <b>63.09%</b>.</div>',
                unsafe_allow_html=True)
    ...
    with tc:
        ...
        wts =c3.selectbox("Weights",["uniform","distance"],key="kw")
        if st.button("▶ Run KNN Classification",use_container_width=True,key="rk"):
            ...
            with st.spinner("Sweeping k…"):
                ...
                accs=[accuracy_score(yte,KNeighborsClassifier(k,weights=wts,n_jobs=-1)
                                     .fit(Xtr,ytr).predict(Xte)) for k in kr]
                bk=kr[int(np.argmax(accs))]; ba=max(accs)
            mbox({"Best k":bk,"Best Acc":f"{ba*100:.2f}%","Split":spl,"Weights":wts})
...
# ══════════════════════════════════════════════════════════════════
    ...
    st.markdown('<div class="card ca">Primary task: predict <b>broad_jump_cm</b>. '
                'Best split: 70:30 → R²=<b>0.8033</b>. '
                'Also adapted for classification (~50% — task mismatch).</div>',
                unsafe_allow_html=True)
    ...
    with tr:
        if st.button("▶ Run Linear Regression",use_container_width=True,key="rlr"):
            ...
            st.subheader("5-Fold CV"); fold_chart(cv5,'R²',1,GREEN)
            mbox({"CV Mean R²":f"{cv5.mean():.4f}","CV Std":f"±{cv5.std():.4f}"})

    with tc:
        st.markdown('<div class="wbox">⚠️ Linear Regression is not designed for '
                    'classification. Continuous outputs binned to quartiles.</div>',
                    unsafe_allow_html=True)
        if st.button("▶ Run Adapted Classification",use_container_width=True,key="rlrc"):
            ...
            for nm,ts in SPLITS:
                ...
                rows.append({'Split':nm,'Adapted Accuracy':
                             f"{accuracy_score([_b(v) for v in yte],[_b(v) for v in yp])*100:.1f}%"})
...
# ══════════════════════════════════════════════════════════════════
    ...
    st.markdown('<div class="card ca">Optimal: α=0.000377 → depth 14, 477 nodes → '
                '<b>72.15%</b> accuracy. Cost-complexity pruning via 5-fold CV.</div>',
                unsafe_allow_html=True)
    tc,tr=st.tabs(["🎯 Classification","📈 Regression"])
    ...
    with tc:
        ...
        maxd =c2.slider("Max depth (0=auto)",0,30,0,key="dtd")
        ...
        if st.button("▶ Run Decision Tree Classification",use_container_width=True,key="rdt"):
            with st.spinner("Training…"):
                ...
                m=DecisionTreeClassifier(random_state=42,ccp_alpha=alpha,max_depth=md)
                m.fit(Xtr,ytr); yp=m.predict(Xte)
            mbox({"Accuracy":f"{accuracy_score(yte,yp)*100:.2f}%",
                  "Depth":m.get_depth(),"Nodes":m.tree_.node_count,"α":f"{alpha:.6f}"})
            c1,c2=st.columns(2)
            ...
            ax.set_title('Overfitting → Underfitting via Alpha'); ax.legend(); ax.grid(alpha=.15)
            show(fig)

            st.subheader("Cross-Split Results")
            res,cv5=run_cls(lambda:DecisionTreeClassifier(random_state=42,
                            ccp_alpha=alpha,max_depth=md),Xc,yc,scale=False)
            st.dataframe(res,use_container_width=True,hide_index=True)
            bar_c(res['Split'][:3],[float(v.split('%')[0]) for v in res['Accuracy'][:3]],
                  'Accuracy (%)','Decision Tree Splits',PURPLE)
            fold_chart(cv5,'Accuracy (%)',100,PURPLE)

            st.subheader("Tree Visualisation (top 3 levels)")
    ...
    with tr:
        c1,c2=st.columns(2)
        dtr_a=c1.slider("ccp_alpha (reg)",0.0,100.0,0.0,key="dtrar")
        dtr_d=c2.slider("Max depth",0,20,0,key="dtrdr"); md2=None if dtr_d==0 else dtr_d
        if st.button("▶ Run DT Regression",use_container_width=True,key="rdtr"):
            with st.spinner("…"):
                res,cv5=run_reg(lambda:DecisionTreeRegressor(random_state=42,
                                ccp_alpha=dtr_a,max_depth=md2),Xr,yr,scale=False)
            st.dataframe(res,use_container_width=True,hide_index=True)
            bar_c(res['Split'][:3],[float(v.split(' ')[0]) for v in res['R²'][:3]],
                  'R²','DT Regression R²',GREEN)
            Xtr,Xte,ytr,yte=train_test_split(Xr,yr,test_size=.2,random_state=42)
            m2=DecisionTreeRegressor(random_state=42,ccp_alpha=dtr_a,max_depth=md2)
            m2.fit(Xtr,ytr); scatter_res(yte,m2.predict(Xte),"DT Regression — Actual vs Predicted")
            fold_chart(cv5,'R²',1,GREEN)
...
# ══════════════════════════════════════════════════════════════════
    ...
    st.markdown('<div class="card ca">Max-margin classifier. Best: '
                '<b>RBF kernel, C=10, γ=0.1 → 71.27%</b>.</div>',
                unsafe_allow_html=True)
    ...
    with tc:
        ...
        if st.button("▶ Run SVM Classification",use_container_width=True,key="rsvm"):
            with st.spinner("Training (30–60s)…"):
                ...
                Xtr,Xte,ytr,yte=train_test_split(Xc,yc,test_size=.2,random_state=42,stratify=yc)
                Xtr=sc.fit_transform(Xtr); Xte=sc.transform(Xte)
                m=SVC(kernel=kern,C=Cval,gamma=gv,random_state=42); m.fit(Xtr,ytr); yp=m.predict(Xte)
            mbox({"Accuracy":f"{accuracy_score(yte,yp)*100:.2f}%",
            ...
            st.subheader("Kernel Comparison (80:20)")
            with st.spinner("Comparing kernels…"):
                kr_res=[]
            ...
            st.subheader("Cross-Split Results")
            res,cv5=run_cls(lambda:SVC(kernel=kern,C=Cval,gamma=gv,random_state=42),Xc,yc)
            st.dataframe(res,use_container_width=True,hide_index=True)
            bar_c(res['Split'][:3],[float(v.split('%')[0]) for v in res['Accuracy'][:3]],
    ...
    with tr:
        ...
        if st.button("▶ Run SVR",use_container_width=True,key="rsvr"):
            ...
            mbox({"R²":f"{r2_score(yte,yp):.4f}",
                  "RMSE":f"{np.sqrt(mean_squared_error(yte,yp)):.2f} cm",
                  "MAE":f"{mean_absolute_error(yte,yp):.2f} cm"})
            scatter_res(yte,yp,"SVR — Actual vs Predicted"); fold_chart(cv5,'R²',1,PURPLE)
...
# ══════════════════════════════════════════════════════════════════
    ...
    st.markdown('<div class="card ca">Architecture: 256→128→64 + ELU + BatchNorm + Dropout. '
                'EarlyStopping (patience=15) + ReduceLROnPlateau. '
                'Best accuracy: <b>74.80%</b> (after leakage fix).</div>',
                unsafe_allow_html=True)
    st.markdown('<div class="wbox">⚠️ <b>Leakage fix:</b> Earlier 100% result was caused '
                'by a preprocessing step embedding target labels. Results here reflect '
                'the corrected leak-free pipeline.</div>', unsafe_allow_html=True)
    ...
    try:
        ...
        TF=True
        st.markdown(f'<div class="sbox">✅ TensorFlow {tf.__version__}</div>',
                    unsafe_allow_html=True)
    except ImportError:
        TF=False
        st.markdown('<div class="wbox">⚠️ Install TensorFlow: '
                    '<code>pip install tensorflow</code></div>',unsafe_allow_html=True)

    ta,tb,tc_=st.tabs(["🎯 Classification","📈 Regression","🏗️ Architecture"])

    with tc_:
        st.dataframe(pd.DataFrame([
            {"Block":"Input",      "Layer":"—",       "Units":len(FC),"Act":"—",     "Reg":"—"},
            {"Block":"Hidden 1",   "Layer":"Dense",   "Units":256,    "Act":"ELU",   "Reg":"BN+Drop(0.20)"},
            {"Block":"Hidden 2",   "Layer":"Dense",   "Units":128,    "Act":"ELU",   "Reg":"BN+Drop(0.15)"},
            {"Block":"Hidden 3",   "Layer":"Dense",   "Units":64,     "Act":"ELU",   "Reg":"BN+Drop(0.10)"},
            {"Block":"Out (Cls)",  "Layer":"Dense",   "Units":4,      "Act":"Softmax","Reg":"—"},
            {"Block":"Out (Reg)",  "Layer":"Dense",   "Units":1,      "Act":"Linear","Reg":"—"},
        ]),use_container_width=True,hide_index=True)
        st.dataframe(pd.DataFrame([
            {"Parameter":"Optimiser",       "Value":"Adam (lr=0.001)"},
            {"Parameter":"Loss (Cls)",       "Value":"Categorical Cross-Entropy"},
            {"Parameter":"Loss (Reg)",       "Value":"MSE"},
            ...
            {"Parameter":"Val Split",        "Value":"15% of training data"},
        ]),use_container_width=True,hide_index=True)
    ...
    if TF:
        def mk_cls(inp,dr):
            ...
            m.compile(optimizer=Adam(.001),loss='categorical_crossentropy',
                      metrics=['accuracy']); return m

        with ta:
            c1,c2,c3=st.columns(3)
            ep_=c1.slider("Max Epochs",30,200,100,key="nep")
            pa_=c2.slider("Patience",5,30,15,key="npa")
            dr_=c3.slider("Dropout",0.1,0.5,.2,step=.05,key="ndr")
            if st.button("▶ Train Neural Network",use_container_width=True,key="rnn"):
                ...
                for nm,ts in SPLITS:
                    with st.spinner(f"Training {nm}…"):
                        np.random.seed(42); tf.random.set_seed(42)
                        Xtr,Xte,ytr,yte=train_test_split(Xc,yc,test_size=ts,
                                                          random_state=42,stratify=yc)
                        sc=StandardScaler(); Xtr=sc.fit_transform(Xtr); Xte=sc.transform(Xte)
                        ytrc=to_categorical(ytr,4)
                        m=mk_cls(Xtr.shape[1],dr_)
                        cb=[EarlyStopping(monitor='val_loss',patience=pa_,
                                          restore_best_weights=True,verbose=0),
                            ReduceLROnPlateau(monitor='val_loss',factor=.5,
                                             patience=5,min_lr=1e-6,verbose=0)]
                        h=m.fit(Xtr,ytrc,epochs=ep_,batch_size=32,
                                validation_split=.15,callbacks=cb,verbose=0)
                        yp_=np.argmax(m.predict(Xte,verbose=0),axis=1)
                        rows_.append({'Split':nm,'Accuracy':
                            f"{accuracy_score(yte,yp_)*100:.2f}%",'Epochs':len(h.history['accuracy'])})
                ...
                with st.spinner("5-fold CV…"):
                    ...
                    for ti,ei in skf.split(Xc,yc):
                        ...
                        s=StandardScaler(); Xt=s.fit_transform(Xt); Xe=s.transform(Xe)
                        ytc=to_categorical(yt,4)
                        mf=mk_cls(Xt.shape[1],dr_)
                        mf.fit(Xt,ytc,epochs=ep_,batch_size=32,
                               validation_split=.15,verbose=0,
                               callbacks=[EarlyStopping(monitor='val_loss',patience=pa_,
                                           restore_best_weights=True,verbose=0)])
                        cv_a.append(accuracy_score(ye,np.argmax(mf.predict(Xe,verbose=0),axis=1)))

                cv_a=np.array(cv_a)
                rows_.append({'Split':'5-Fold CV',
                    'Accuracy':f"{cv_a.mean()*100:.2f}% ±{cv_a.std()*100:.2f}",'Epochs':'—'})
                ...
                ax[1].legend(); ax[1].grid(alpha=.15); show(fig)
        ...
        with tb:
            if st.button("▶ Train NN Regression",use_container_width=True,key="rnnr"):
                def mk_reg(inp,dr):
                    np.random.seed(42); tf.random.set_seed(42)
                    m=Sequential([
                        Dense(256,input_dim=inp,use_bias=False),BatchNormalization(),
                        Activation('elu'),Dropout(dr),
                        Dense(128,use_bias=False),BatchNormalization(),
                        Activation('elu'),Dropout(dr*.75),
                        Dense(64,use_bias=False),BatchNormalization(),
                        Activation('elu'),Dropout(dr*.5),Dense(1,activation='linear')])
                    m.compile(optimizer=Adam(.001),loss='mse',metrics=['mae']); return m
                rr=[]
                for nm,ts in SPLITS:
                    with st.spinner(f"Regression {nm}…"):
                        ...
                        mr.fit(Xtr,yts,epochs=100,batch_size=32,validation_split=.15,verbose=0,
                               callbacks=[EarlyStopping(monitor='val_loss',patience=15,
                                           restore_best_weights=True,verbose=0)])
                        yp_=sY.inverse_transform(mr.predict(Xte,verbose=0)).ravel()
                        rr.append({'Split':nm,'R²':f"{r2_score(yte,yp_):.4f}",
                ...
                bar_c([r['Split'] for r in rr],[float(r['R²']) for r in rr],
                      'R²','NN Regression R²',BLUE)
...
# ══════════════════════════════════════════════════════════════════
    ...
    st.dataframe(pd.DataFrame([
        {"Model":"KNN (k=22)",            "Best Acc":"63.09%","F1":"0.63","Split":"80:20","5-CV":"61.9%","Note":"❌"},
        {"Model":"Decision Tree (pruned)","Best Acc":"72.15%","F1":"0.71","Split":"80:20","5-CV":"70.39%","Note":"✅"},
        {"Model":"SVM (RBF C=10)",        "Best Acc":"71.27%","F1":"0.71","Split":"80:20","5-CV":"70.8%","Note":"✅"},
        {"Model":"Neural Network",        "Best Acc":"74.80%","F1":"~0.74","Split":"70:30","5-CV":"73.98%","Note":"⭐ Best"},
        {"Model":"Linear Reg. (adapted)", "Best Acc":"~50%",  "F1":"~0.48","Split":"N/A", "5-CV":"N/A","Note":"❌"},
    ]),use_container_width=True,hide_index=True)
    st.markdown('</div>', unsafe_allow_html=True)
    ...
    st.dataframe(pd.DataFrame([
        {"Model":"Neural Network",   "Best R²":"0.802","RMSE":"17.77 cm","Split":"80:20","Note":"⭐ Best"},
        {"Model":"Linear Regression","Best R²":"0.8033","RMSE":"17.89 cm","Split":"70:30","Note":"✅ Simplest"},
    ...
    with c1:
        ...
        bars=ax.bar(models_c,accs_c,color=PAL+[LBLUE],edgecolor=DARK,width=.55,zorder=3)
        for b in bars:
            ax.text(b.get_x()+b.get_width()/2,b.get_height()+.5,
                    f'{b.get_height():.1f}%',ha='center',color='#C8D8EA',fontsize=9)
        ax.set_ylim(0,90); ax.set_ylabel('Best Accuracy (%)')
        ax.set_title('Classification Accuracy')
        ax.axhline(74.8,color=GREEN,lw=1.5,linestyle='--',alpha=.7,label='Best 74.8%')
        ax.legend(); ax.grid(axis='y',alpha=.15); show(fig)
    with c2:
        fig,ax=plt.subplots(figsize=(7,4.5))
        models_r=["Neur.Net","Lin.Reg","SVM","KNN","Dec.Tree"]
        r2s=[.802,.8033,.79,.788,.76]
        bars=ax.bar(models_r,r2s,color=[GREEN,BLUE,PURPLE,LBLUE,RED],edgecolor=DARK,width=.55,zorder=3)
        for b in bars:
            ax.text(b.get_x()+b.get_width()/2,b.get_height()+.003,
                    f'{b.get_height():.3f}',ha='center',color='#C8D8EA',fontsize=9)
        ax.set_ylim(.5,1.); ax.set_ylabel('R² Score')
        ax.set_title('Regression R²'); ax.grid(axis='y',alpha=.15); show(fig)
    st.markdown('</div>', unsafe_allow_html=True)

    st.markdown('<div class="card">', unsafe_allow_html=True)
    st.subheader("🕸️ Radar Chart — Classification")
    cats=['Accuracy','Precision','Recall','F1','CV Score']
    scores_r={"KNN":[63,65,63,63,62],"Dec.Tree":[72,72,71,71,70],
              "SVM":[71,71,71,71,71],"MLP":[75,74,75,74,74]}
    ...
    for (nm,sc),col in zip(scores_r.items(),PAL):
        v=sc+sc[:1]; ax.plot(angles,v,color=col,lw=2.2,label=nm); ax.fill(angles,v,color=col,alpha=.1)
    ...
    for l in ax.xaxis.get_gridlines(): l.set_color('#1D4570')
    ...
    st.subheader("⚠️ ~71% Accuracy Ceiling — Root Cause")
    st.markdown("""
| Root Cause | Detail |
|---|---|
| **Class B/C overlap** | Adjacent tiers share physiological ranges — F1 for B (0.48–0.58) is consistently lowest |
| **Low-signal features** | Diastolic/systolic r<0.07; feature ablation shows ≤1% gain from removal |
| **Label ambiguity** | Classes discretised from continuous scores; boundary participants could belong to either tier |
| **Tuning performed** | SVM C=0.01→100 × 3 kernels; DT alpha CV; KNN k=1→99; MLP 3 depths, dropout 0.1–0.5 |

**Best legitimate accuracy: MLP = 74.80%** (corrected from 100% data-leakage bug).
    """)
    st.markdown('</div>', unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════
    ...
    st.markdown('<div class="card ca">Enter measurements to predict '
                '<b>Performance Class (A–D)</b> and <b>Broad Jump distance</b>.</div>',
                unsafe_allow_html=True)
    ...
    with st.form("pf"):
        ...
        bj_ =c3.number_input("Broad Jump (cm, optional)",0.,310.,190.,step=1.,
                              help="Leave at 190 if unknown")
        mc_ =st.selectbox("Model",["KNN (k=22)","Decision Tree","SVM (RBF, C=10)"])
    ...
    if sub_:
        ...
        with st.spinner("Predicting…"):
            if "KNN" in mc_:
                ...
                xci=sc_c.transform(xc); xri=sc_r.transform(xr)
        ...
        cn=CL[pc]
        info={'A':("🥇 Excellent",GREEN,"Top tier. Maintain your programme."),
              'B':("🥈 Good",BLUE,"Above average. Focus on strength."),
              'C':("🥉 Average",PURPLE,"Moderate. Increase endurance training."),
              'D':("⚠️ Below Avg",RED,"Below average. Start a structured programme.")}
        lbl,col,adv=info[cn]
        st.markdown(f'<div class="card" style="border:2px solid {col};text-align:center;padding:28px">'
                    f'<div style="font-size:3rem">{lbl}</div>'
                    f'<div style="font-size:1.5rem;font-weight:900;color:{col}">Class: {cn}</div>'
                    f'<div style="color:#C8D8EA;margin-top:6px">{adv}</div></div>',
                    unsafe_allow_html=True)
        mbox({"Class":cn,"Pred. Jump":f"{pj:.1f} cm","BMI":f"{bmi_:.1f}",
              "Model":mc_.split()[0]})
        ...
        for lo,hi,c,lb in [(0,120,RED,'<120'),(120,165,'#E8963A','120–165'),
            ...
            ax.text((lo+hi)/2,.53,lb,ha='center',va='bottom',color='#C8D8EA',fontsize=8)
        ax.axvline(pj,color='white',lw=3,zorder=5,label=f'{pj:.0f} cm')
        ax.set_xlim(0,320); ax.set_ylim(-.2,.9); ax.set_yticks([])
        ax.set_xlabel('Broad Jump (cm)'); ax.set_title('Jump Gauge'); ax.legend(); show(fig)

        # Profile comparison
        st.subheader("👤 Your Profile vs Dataset Averages")
        fn=['Age','Height','Weight','Body Fat%','Diastolic','Systolic',
            'Grip','Sit&Bend','Sit-ups','BMI']
        yv=[age_,ht_,wt_,bf_,dia_,sys_,gr_,fl_,su_,round(bmi_,1)]
        mv=[df['age'].mean(),df['height_cm'].mean(),df['weight_kg'].mean(),
        ...
        ax.bar(x_+.22,mv,.42,label='Mean',color=PURPLE,edgecolor=DARK,alpha=.75,zorder=3)
...
# ─── Footer ──────────────────────────────────────────────────────
st.markdown("""
<hr style="border:none;border-top:1px solid #1D4570;margin-top:48px">
<div style="text-align:center;padding:16px 0;color:#4d7fa8;font-size:.82rem">
  <strong style="color:#3B8BD4;font-size:.95rem">AXORA</strong> · Body Performance Analytics
  &amp; Intelligent Classification System<br>
  <span style="color:#85B7EB">Alaa Issawi · Amira Salama · Aya Abdel Maksoud ·
  Aya El-Sabi · Aya Imam · Aya Khalil</span><br>
