...
Version 8.0 | Team Axora | Introduction to AI & ML | 2024-2025
"""
...
if page == "🏠 Home":
    ...
    with c2:
        st.markdown(_logo(400, 248), unsafe_allow_html=True)
        st.markdown("""
        <div style="margin-top:18px">
          <div style="font-size:1.7rem;font-weight:900;color:#3B8BD4">Body Performance Analytics</div>
          <div style="font-size:1rem;color:#85B7EB;margin-top:6px">Intelligent Classification &amp; Regression System</div>
          <div style="color:#4d7fa8;font-size:.82rem;margin-top:6px">5 Machine Learning Models · Team Axora · 2024–2025</div>
...
# ══════════════════════════════════════════════════════
    ...
    # ── Decision Tree ─────────────────────────────────
        ...
        st.markdown(
            ...
            '<b>Regression:</b> α=16.21 → depth 3 → <b>R²=0.677</b>.</div>', unsafe_allow_html=True
        )
        st.markdown('<div class="card cb">', unsafe_allow_html=True)
        st.subheader("📒 Notebook Results (Verified)")
        mbox({"Best Cls Acc": "71.07%", "CV Cls Acc": "70.39%",
              "Best Reg R²": "0.677",   "CV Reg R²": "0.695"})
    ...
    # ── SVM ───────────────────────────────────────────
        ...
        st.markdown(
            '<div class="card cg">Maximum-margin classifier. '
            'RBF kernel (C=100, γ=0.01) → <b>71.31%</b> on full dataset. '
            'SVR regression: R²≈<b>0.797</b>.</div>', unsafe_allow_html=True
        )
        st.markdown('<div class="card cb">', unsafe_allow_html=True)
        st.subheader("📒 Notebook Results (Verified)")
        mbox({"Linear (full)": "63.13%", "RBF Tuned (full)": "71.31%",
              "Best Params": "C=100, γ=0.01", "Kernel": "RBF"})
        ...
        st.markdown(
            '<div class="wbox">📌 CV classification is 67.19% on full dataset with grid search '
            'best params (C=100, γ=0.01). SVR RBF achieves R²≈0.797–0.798 across all splits.</div>',
            unsafe_allow_html=True
        )
        st.markdown("</div>", unsafe_allow_html=True)
